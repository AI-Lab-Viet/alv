...
import { CheckCircle, Shield, AlertTriangle, Award, MapPin, BookOpen } from 'lucide-react';
    ...
    case 'intro':
      return (
        <div className='text-center space-y-6'>
          ...
          <div className='pt-4'>
            <Button onClick={onStateTransition} className='w-full' disabled={currentState > 0}>
                ...
                <div className='flex items-start gap-4'>
                  ...
                  <div className='flex-1'>
                    {contentDisplay.lessonBlocks
                      .sort((a: LessonBlock, b: LessonBlock) => a.display_order - b.display_order)
          ...
          <Card className='bg-yellow-50 border-yellow-200'>
            <CardContent className='p-6'>
              <div className='flex items-start gap-4'>
                ...
                <div className='flex-1'>
                  <h4 className='font-semibold text-foreground mb-4'>Bộ câu hỏi Vàng</h4>
                  <div className='space-y-3'>
                    {contentDisplay.questions.map((question: string, index: number) => (
                      <div key={index} className='p-3 bg-white rounded-lg border border-yellow-200'>
                        <p className='text-sm font-medium text-yellow-800'>{question}</p>
                      </div>
                    ))}
                  </div>
                </div>
    ...
    case 'clickable_text':
      return (
        <div className='space-y-6'>
          <div className='text-center'>
            <h3 className='text-xl font-semibold text-foreground mb-2'>{contentDisplay.title}</h3>
            <p className='text-sm text-muted-foreground'>{contentDisplay.instruction}</p>
          </div>
          <Card className='bg-blue-50 border-blue-200'>
            <CardContent className='p-6'>
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html:
                    contentDisplay.clickable_words?.reduce((text: string, word: string) => {
                      return text.replace(
                    ...
                    }, contentDisplay.text || '') || contentDisplay.text
                }}
              />
              {clickedError && (
                <div className='mt-4 p-3 bg-green-100 border border-green-200 rounded-lg'>
                  <p className='text-sm text-green-700'>✓ Chính xác! Bạn đã tìm ra lỗi sai.</p>
                </div>
              )}
              {wrongClickedWord && !clickedError && (
                <div className='mt-4 p-3 bg-red-100 border border-red-200 rounded-lg'>
                  <p className='text-sm text-destructive'>✗ Chưa đúng. Hãy thử lại!</p>
                </div>
              )}
            </CardContent>
          </Card>
    ...
    case 'multiple_choice':
      return (
        <div className='space-y-6'>
          <div className='text-center'>
            <h3 className='text-xl font-semibold text-foreground mb-2'>{contentDisplay.title}</h3>
            <p className='text-sm text-muted-foreground'>{contentDisplay.question}</p>
          </div>
        ...
        <div className='space-y-6'>
          ...
          <Card className='bg-blue-50 border-blue-200'>
            <CardContent className='p-6'>
              ...
              <p className='text-sm text-muted-foreground'>{contentDisplay.prompt}</p>
            </CardContent>
          </Card>
        </div>
      );
    ...
    case 'completion':
      return (
        <div className='text-center space-y-6'>
          ...
          <div className='space-y-3'>
            <Button onClick={() => onRouterPush('/skill-hub')} className='w-full'>
